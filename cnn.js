...
app.get('/feedme', (req, res, next) => {
    res.send('Make yourself dinner');
});

app.options('/saveimage/:name(bread|notbread)', cors(corsOptions))
app.post('/saveimage/:name(bread|notbread)', cors(corsOptions), (req, res, next) => {
	...
	const filepath = path.join(__dirname, `./${req.params.name}/${new Date() * 1}.${type}`);
